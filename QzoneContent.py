...
def get_content(file_path,qq):
    ...
    if access == True:    #有权限访问，则利用selenium获取说说信息
        ...
        content = driver.find_elements_by_css_selector('.content')    #搜索指定标签
        detail = driver.find_elements_by_css_selector('.c_tx.c_tx3.goDetail')    #搜索指定标签
        for con,det in zip(content,detail):    #将说说信息写入文件
            fwrite.write('时间:'+det.text+'     链接:'+det.get_attribute('href')+'\n')
            fwrite.write('内容:'+con.text+'\n')
    try:
...
if __name__ == '__main__':
    get_content('data/Content.txt','QQ')